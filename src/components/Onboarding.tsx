...
export default function Onboarding({ onComplete }: Props) {
  ...
  // Steps: 0=silhouette, 1=style, 2=lifestyle, 3=budget, 4=brands, 5=makeup, 6=avatar
  const canProceed = [
    silhouette !== '',   // 0
    styles.length > 0,   // 1
    lifestyle !== '',     // 2
          ...
          <>
            ...
            {brands.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-4">
                {brands.map(b => (
                  <span key={b} onClick={() => removeBrand(b)} className="chip chip-active cursor-pointer">
                    {b} ✕
                  </span>
                ))}
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              {BRAND_SUGGESTIONS.filter(b => !brands.includes(b)).map(b => (
                <button key={b} onClick={() => addBrand(b)} className="chip" disabled={brands.length >= 3}>
                  {b}
                </button>
              ))}
            </div>
