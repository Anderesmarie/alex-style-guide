              ...
              {filteredWardrobe.length === 0 ? (
                ...
                <div className="grid grid-cols-3 gap-2">
                  {filteredWardrobe.map(item => (
                    ...
                    >
                      <img src={getThumb(item.imageBase64, 400)} alt={item.type} className="w-full h-full object-contain" />
                    </button>
                  ))}
                </div>
              )}
        ...
        <div className="grid grid-cols-3 gap-2 mt-4 mb-4">
          {items.map(item => (
            <div key={item.id} className="rounded-lg overflow-hidden card-shadow">
              <img src={getThumb(item.imageBase64, 400)} alt={item.type} className="w-full aspect-square object-cover bg-white" />
